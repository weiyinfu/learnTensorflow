...
多GPU卷积神经网络最主要的逻辑在于反向传播计算梯度的时候需要手动求多个GPU上的梯度均值

即:optimizer.compute_gradients和optimizer.apply_gradients
...
def conv_net(x, n_classes, dropout, reuse, is_training):
    ...
    with tf.variable_scope('ConvNet', reuse=reuse):
        ...
        x = tf.reshape(x, shape=[-1, 28, 28, 1])  # 将输入数据展成28*28深度为1的图片

        # Convolution Layer with 64 filters and a kernel size of 5
        x = tf.layers.conv2d(x, 64, 5, activation=tf.nn.relu)  # 卷积层使用核为5的卷积，卷继承深度为64的图片
        # Max Pooling (down-sampling) with strides of 2 and kernel size of 2
        x = tf.layers.max_pooling2d(x, 2, 2)

        # 两层卷积、一层池化，卷积层也要有激活函数，不然卷积没有意义，就像多个线性分类器叠加还是线性分类器一样
...
with tf.device('/cpu:0'):  # 默认使用CPU
    ...
    for i in range(num_gpus):
        ...
        with tf.device('/gpu:%d' % i):
            ...
            # Because Dropout have different behavior at training and prediction time, we need to create 2 distinct computation graphs that share the same weights.
            ...
            self_opetimizer = tf.train.AdamOptimizer(learning_rate=learning_rate)
            # Define loss and optimizer (with train logits, for dropout to take effect)
            loss_op = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=logits_train, labels=_y))
            """
            这个地方非常关键：
            多GPU训练神经网络关键在于需要计算梯度的均值
            以前是直接minimize反馈回去了，而这里需要先计算出来
            """
            grads = self_opetimizer.compute_gradients(loss_op)
    ...
    """
    多个GPU上的计算图共享权值，但是不同GPU上分布着不同的计算结点
    计算多个GPU上的梯度均值，只让一个GPU执行反馈过程
    optimizer只存储在一个GPU上
    """
    ...
    with tf.Session() as sess:
        ...
        for step in range(num_steps + 1):
            ...
            if step % display_step == 0:
                ...
                print("Step ", step, "Minibatch Loss= ", "{:.4f}".format(loss) + ", Training Accuracy= ", "{:.3f}".format(acc) + ", %i Examples/sec" % int(len(batch_x) / te))

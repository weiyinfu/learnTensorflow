...
place_x = tf.placeholder(dtype=tf.int32, shape=(None, None))
place_y = tf.placeholder(dtype=tf.int32, shape=(None, 1))
"""
...
embed_x = tf.nn.embedding_lookup(embedding, place_x)
"""
错误思路一：把y映射到词向量，使用范数距离。最后全部单词的嵌入都会压缩到一个点
embed_y = tf.nn.embedding_lookup(embedding, place_y)
loss = tf.norm(ctx - embed_y)

错误思路二：使用另一个矩阵w和b映射到y，计算logits
最后embedding可能并不完美，主要信息都存在于w和b里面

w = tf.Variable(tf.random_normal((embedding_size, num_words)), trainable=False)
b = tf.Variable(tf.random_normal((num_words,)), trainable=False)
logits = tf.matmul(ctx, w) + b
"""
ctx = tf.reduce_mean(embed_x, axis=1)  # 求均值表示上下文，此处不管是mean还是sum效果似乎差不多

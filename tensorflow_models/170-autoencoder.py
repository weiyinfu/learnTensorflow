...
这个程序编码阶段和解码阶段没有共享权值,有篇论文指出:是可以共享权值的
...
decoder_op = decoder(encoder_op)  # decoder操作是先加密再解密，而不是单单的解密
...
with tf.Session() as sess:
    ...
    for i in range(n):
        ...
        for j in range(n):
            ...
            canvas_reconstructed[i * 28:(i + 1) * 28, j * 28:(j + 1) * 28] = g[j].reshape([28, 28])

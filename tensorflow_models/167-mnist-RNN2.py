...
def model(X, W, B, lstm_size):
    # 注意学习数据变形的过程:把数据弄成frame_count*frame_size的形式
    ...
    lstm = BasicLSTMCell(lstm_size, forget_bias=1.0, state_is_tuple=True)

    # Get lstm cell output, time_step_size (28) arrays with lstm_size output: (batch_size, lstm_size)
    outputs, _states = static_rnn(lstm, X_split, dtype=tf.float32)
...
# 这个选项使得GPU以增量的方式占用内存而不是一下子占据全部内存
